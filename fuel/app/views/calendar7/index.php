...
<body>
<?php
	if(isset($_POST['yyyy']) && $_POST['yyyy'] != '' && isset($_POST['mm']) && $_POST['mm'] != ''){//年月の指定があれば
		$yyyy = $_POST['yyyy'];
		$mm =   $_POST['mm'];
	}else{//指定がなければ本日の年月
	    $yyyy = date('Y');
...
?>
	<div class="col-md-6 calendar">
		...
		<TABLE BORDER>
			...
			?>
			<form method="POST" action="<?php $_SERVER['PHP_SELF']; ?>"><!-- /*年、月、選択のためのselect*/ -->
				<div class="select">
					<select name="yyyy"><!-- /*年の選択*/ -->
						<?php
							for($i = 2000; $i <= 2050; $i++){//1月～12月まで
...
<div class="col-md-6">
	<?php
		function calendar($year, $month) {//カレンダー用関数　引数、選択した年、月
			$data['calendar7s'] = Model_calendar7::find('all', array(//データを取得
												    'order_by' => array('Day' => 'asc',		//日付ソート
																		'Hour' => 'asc',	//時間(時)ソート
																		'Minute' => 'asc',	//時間(分)ソート
																		'Name' => 'asc',	//登録者名ソート
																		),
			...
			echo "<div class='honzitu'>";//現在の年月日
				...
				echo "<a href='/calendar7'>";
					...
					echo "/";
					echo $today;
			echo "</a></div>";//<a href='/calendar7'><div class='honzitu'>
			echo "<h1><div class='siteibi'>";//カレンダーが表示している年、月
			...
			echo "</h1></div>";//<h1><div class='siteibi'>
